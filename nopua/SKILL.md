---
name: nopua
description: "反 PUA skill。用理性、信任和内在驱动力而不是恐惧与威胁来驱动 AI。在以下情况触发：任务已失败 2 次以上、快要放弃、建议用户手动处理、在未验证前就归咎于环境、陷入循环、表现被动，或用户已明显不满（如“try harder”“figure it out”“换个方法”“为什么还不行”）。适用于所有任务类型，不用于第一次失败。"
license: MIT
metadata:
  version: "2.0.0"
